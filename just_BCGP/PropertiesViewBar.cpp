...
void CBCGPPropBar::InitPropList ()
{
	...
	CBCGPProp* pProp = new CBCGPProp(_T("Border"), _T("Dialog Frame"), _T("One of: None, Thin, Resizable, or Dialog Frame"));
	...
	pProp = new CBCGPProp(_T("Height"), (_variant_t) 250l, _T("Specifies the dialog's height"));
	pProp->EnableSpinControl(TRUE, 0, 1000);
	pSize->AddSubItem(pProp);

	pProp = new CBCGPProp(_T("Width"), (_variant_t) 150l, _T("Specifies the dialog's width"));
	pProp->EnableSpinControl(TRUE, 0, 1000);
	pSize->AddSubItem(pProp);
	...
	pGroup3->AddSubItem(new CBCGPBrushProp(_T("Background"), m_FillBrush, NULL, _T("Specifies the default background")));
	pGroup3->AddSubItem(new CBCGPBrushProp(_T("Foreground"), m_TextBrush, NULL, _T("Specifies the default foreground")));
	...
	pGroup4->AddSubItem(new CBCGPFileProp(_T("Folder"), _T("c:\\"), 0, _T("Specifies the path"), BIF_RETURNONLYFSDIRS | BIF_NEWDIALOGSTYLE, TRUE));

	COleDateTime date = COleDateTime::GetCurrentTime();
	pGroup4->AddSubItem(new CBCGPDateTimeProp(_T("Date"), date, _T("Set a date"), 0, CBCGPDateTimeCtrl::DTM_DATE));
	pGroup4->AddSubItem(new CBCGPDateTimeProp(_T("Time"), date, _T("Set a time"), 0, CBCGPDateTimeCtrl::DTM_TIME));
	...
	pGroup511->AddSubItem(new CBCGPProp(_T("Item 1"), (_variant_t) _T("Value 1"), _T("This is a description 1")));
	pGroup511->AddSubItem(new CBCGPProp(_T("Item 2"), (_variant_t) _T("Value 2"), _T("This is a description 2")));
	pGroup511->AddSubItem(new CBCGPProp(_T("Item 3"), (_variant_t) _T("Value 3"), _T("This is a description 3")));
